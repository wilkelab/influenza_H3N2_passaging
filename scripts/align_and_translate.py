...
def translate_records(nuc_dict):
    ...
    for entry in nuc_dict:
        ...
        if len(nucseq)%3 !=0:
	 	print "{0} nucleotide sequence  was not divisible by 3. Skipped".format(entry) 
		break
	else:
		...
		for i in range(0,len(nucseq),3):
			...
			if i == len(nucseq)-3:
			    ...
			    print "\n\n You have internal stop codons at position {0} in {1} quitting.".format(i, entry)
			    break
		    else:
			prot_seq += amino
...
def main():
    ...
    print "\nComplete! Your final protein alignment is in", args.prot_outfile, "and your final nucleotide alignment is in", args.nuc_outfile
